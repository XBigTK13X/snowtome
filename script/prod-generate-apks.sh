...
function build_variant() {
    ...
    if [ "$out_ext" = "apk" ]; then
        cp app/build/outputs/apk/release/app-release.apk "build-out/snowtome-$variant_name.apk"
    else
        cp app/build/outputs/bundle/release/app-release.aab "build-out/snowtome-$variant_name.aab"
    fi
